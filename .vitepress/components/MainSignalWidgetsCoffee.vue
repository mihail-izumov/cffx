...
// Конфигурация времени ответа (как в старом коде)
const cafeProfiles = {
...
// Иконка и дата (восстановлена из старого кода)
const todayStatus = computed(() => {
  ...
  // Восстановленная полная SVG иконка
...
// Модальные окна
const isSignalModalOpen = ref(false) // Для "Отправить сигнал"
const isVoteModalOpen = ref(false)   // Для "Оценить место"

const showYandexTooltip = ref(false)
const showSignalsTooltip = ref(false)
const showInfoModal = ref(false) // Инфо-модалка (i)

// Логика таймера (восстановлена из старого кода: 45 сек, loadFactor)
...
onMounted(() => {
  ...
  // Таймер обновляется раз в 45 секунд, как было в старом дизайне
...
<template>
  <div class="signal2-widget-content" ref="widgetContentRef">
    <!-- Переключатели (без изменений, как в старом дизайне) -->
      ...
      <div class="signal2-main-card">
        ...
        <div class="signal2-stats-grid">
          ...
          <div class="signal2-stat-card signal2-lime-stat" @mouseenter="!isMobile ? showSignalsTooltip = true : null" @mouseleave="!isMobile ? showSignalsTooltip = false : null">
            <div class="signal2-stat-content">
              ...
              <!-- signal-100-badge возвращает белый цвет текста и анимацию -->
        ...
        <div class="signal2-control-panel">
          <div class="signal2-control-panel-header">
            ...
            >
              <!-- Восстановленная круглая иконка i -->
    ...
    >
      <div class="signal2-review-modal-content" @click.stop>
        ...
        <div class="signal2-modal-close-section">
          <button @click="closeVoteModal" class="signal2-modal-close-button">Закрыть и вернуться</button>
        </div>
      </div>
    </div>
...
/* Стили модального окна (восстановлены из paste-2.txt) */
...
/* ОБНОВЛЕНО: Размер текста и отступы */
.signal2-stat-value { font-family: 'Inter', sans-serif; font-size: 2.2rem; font-weight: 600; line-height: 1; color: #fff; margin-bottom: 4px; text-shadow: 0 0 20px rgba(0, 0, 0, 0.7), 0 0 10px rgba(0, 0, 0, 0.7); transition: transform 0.4s cubic-bezier(0.2, 0.8, 0.2, 1); }
...
/* Label цвет изменен на серый и отступы скорректированы */
.signal2-stat-label { color: #888; font-weight: 700; font-size: 14px; letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 12px; transition: transform 0.4s cubic-bezier(0.2, 0.8, 0.2, 1); }
...
/* Восстановленный стиль кнопки info */
.signal2-info-button { 
...
@media (max-width: 768px) {
  ...
  /* Размер шрифта для мобильных уменьшен */
  .signal2-stat-value { 
  ...
  /* Кнопка "Закрыть и вернуться" снова видна и на своем месте */
...
.signal2-modal-close-icon {
  display: flex !important;
  position: absolute !important; /* было fixed, сделал absolute для привязки к окну */
  top: 12px !important;
...
.signal-100-badge .signal2-badge-text {
  color: #fff; /* Белый цвет текста для 100% решения */
  font-weight: 700;
}

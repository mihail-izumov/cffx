<template>
  <section class="signal-sla dark compact">
    ...
    <div class="card">
      ...
      <div class="company-fields">
        <label class="row big-input">
          <input style="display:none" /><span>Название компании</span>
        </label>
        ...
        <label class="row">
          ...
          <input class="range long white" type="range"
                 :min="sliderGuestsMin" :max="sliderGuestsMax" :step="sliderGuestsStep"
                 v-model.number="state.company.guests_or_clients" />
        ...
        <label class="row">
          ...
          <input class="range long white" type="range"
                 :min="sliderMoneyMin" :max="sliderMoneyMax" :step="sliderMoneyStep"
                 v-model.number="state.company.avg_check_or_subscription" />
        ...
        <div class="ltv-ret-block">
          ...
          <div class="ltv-block">
            ...
            <input v-if="state.company.ltv_cards.includes('Другое')" v-model="state.company.ltv_tool_other" type="text" class="fullwidth ltv-other"
    ...
    <div class="card">
      ...
      <div class="nps-cards">
        <button v-for="c in npsCards" :key="c.value"
                class="nps-card" :class="{active: state.nps.step===c.value}" @click="selectNps(c.value)">
          {{ c.label }}
        </button>
      </div>
    ...
    <div class="card">
      ...
      <div class="mini-ag">
        <div class="mini-badge">
          Кат. А — 4 часа
          <div class="mini-sub">{{ ownerLabel(state.categories_map.A.owner) }}</div>
        </div>
        <div class="mini-badge">
          Кат. Б — 2 часа
          <div class="mini-sub">{{ ownerLabel(state.categories_map.B.owner) }}</div>
        </div>
        <div class="mini-badge">
          Кат. В — 1 час
          <div class="mini-sub">{{ ownerLabel(state.categories_map.C.owner) }}</div>
        </div>
        <div class="mini-badge">
          Кат. Г — 15 минут
          <div class="mini-sub">{{ ownerLabel(state.categories_map.D.owner) }}</div>
        </div>
      </div>
    ...
    <div class="card">
      <h3>Шаблон тикета</h3>
      <div class="goal-line">
        Базовые поля: {{ state.ticket_template.base_fields_ru.join(', ') }}
      </div>
      <div class="goal-line">
        Дополнительные поля: {{ state.ticket_template.extra_fields.join(', ') || 'не выбрано' }}
      </div>
          ...
          <div class="pricing-modal-window" @click.stop>
              ...
              <div class="pricing-modal-body">
                <div class="owner-grid spaced">
                  <div v-for="k in ['A','B','C','D']" :key="k" class="owner-col surface">
                    ...
                    <label v-if="(state.categories_map as any)[k].owner==='custom'" class="row surface">
                      <input style="display:none" />
                      <span>Контакт</span>
                    ...
                    <div class="topics-grid compact3">
                      ...
                      >
                        ...
                        <span class="t-name black">{{ name }}</span>
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <template v-else-if="modalKind==='ticket'">
              ...
              <div class="pricing-modal-body">
                ...
                <div class="extras-grid">
                  <button v-for="f in availableExtraFields()" :key="f" type="button"
                          class="extra-card" :class="{active: state.ticket_template.extra_fields.includes(f)}"
                          @click="toggleExtraField(f)">
                    {{ f }}
                  </button>
                </div>
            ...
            <template v-else-if="modalKind==='goals_ops'">
              ...
              <div class="pricing-modal-body spaced-large">
                <div class="goal-block surface black">
                  <label class="row surface"><input style="display:none" /><span>Полное закрытие (ч): <strong>{{ state.goals.full_close_time_hours }}</strong></span></label>
                  <input class="range long white" type="range" min="1" max="24" v-model.number="state.goals.full_close_time_hours" />
                </div>
                <div class="goal-block surface black">
                  <label class="row surface"><input style="display:none" /><span>Без эскалации: <strong>{{ state.goals.resolved_without_escalation_pct }}%</strong></span></label>
                  <input class="range long white" type="range" min="0" max="100" v-model.number="state.goals.resolved_without_escalation_pct" />
                </div>
              </div>
            </template>
            ...
            <template v-else-if="modalKind==='goals_quality'">
              ...
              <div class="pricing-modal-body spaced-large">
                <div class="goal-block surface black">
                  <label class="row surface">
                    <input style="display:none" />
                    <span>Точность рекомендаций: <strong>{{ state.goals.reco_accuracy_pct }}%</strong></span>
                  </label>
                  <input class="range long white" type="range" min="0" max="100" v-model.number="state.goals.reco_accuracy_pct" />
                </div>

                <div class="goal-block surface black">
                  <label class="row surface">
                    <input style="display:none" />
                    <span>Получение NPS: <strong>{{ state.goals.nps_collected_pct }}%</strong></span>
                  </label>
                  <input class="range long white" type="range" min="0" max="100" v-model.number="state.goals.nps_collected_pct" />
                </div>

                <div class="goal-block surface black">
                  <label class="row surface">
                    <input style="display:none" />
                    <span>Средний NPS: <strong>{{ state.goals.nps_avg }}</strong></span>
                  </label>
                  <input class="range long white" type="range" min="1" max="10" v-model.number="state.goals.nps_avg" />
                </div>
              </div>
            </template>
            ...
            <template v-else-if="modalKind==='goals_business'">
              ...
              <div class="pricing-modal-body spaced-large">
                <div class="goal-block surface black">
                  <label class="row surface">
                    <input style="display:none" />
                    <span>Возврат после жалобы: <strong>{{ state.goals.returns_after_complaint_pct }}%</strong></span>
                  </label>
                  <input class="range long white" type="range" min="0" max="100" v-model.number="state.goals.returns_after_complaint_pct" />
                </div>

                <div class="goal-block surface black">
                  <label class="row surface">
                    <input style="display:none" />
                    <span>Средняя компенсация (₽): <strong>{{ state.goals.avg_compensation_rub }}</strong></span>
                  </label>
                  <input class="range long white" type="range" min="0" max="5000" step="10" v-model.number="state.goals.avg_compensation_rub" />
                </div>
              </div>
            </template>
            ...
            <template v-else-if="modalKind==='sla_later'">
              <div class="pricing-modal-header">СТРУКТУРА</div>
              <h2 class="pricing-modal-title">потом</h2>
              <div class="pricing-modal-body">
                <div class="surface pad black">
                  {{ SLA_LATER.join(', ') }}
                </div>
            ...
            <template v-else>
              ...
              <div class="pricing-modal-body spaced-large">
                <div class="grid2">
                  <div class="surface pad black">
                    <h4>Будни</h4>
                    <label class="row surface"><input style="display:none" /><span>От</span><input v-model="state.work_hours.weekdays.from" type="time" /></label>
                    <label class="row surface"><input style="display:none" /><span>До</span><input v-model="state.work_hours.weekdays.to" type="time" /></label>
                  </div>
                  <div class="surface pad black">
                    <h4>Выходные</h4>
                    <label class="row surface"><input style="display:none" /><span>От</span><input v-model="state.work_hours.weekends.from" type="time" /></label>
                    <label class="row surface"><input style="display:none" /><span>До</span><input v-model="state.work_hours.weekends.to" type="time" /></label>
                  </div>
                </div>
              </div>
            </template>
...
h2{font-size:22px;margin:0 0 6px} h3{font-size:16px;margin:0 0 8px} h4{font-size:14px;margin:0 0 6px}
.subtitle{margin-top:-6px;color:var(--muted)} .subtitle.big{font-size:14px}
...
.range{width:100%;appearance:none;background:transparent;height:24px}
...
.range.white::-webkit-slider-thumb{appearance:none;width:16px;height:16px;border-radius:50%;background:#ffffff;margin-top:-6px;border:none}
.range.white::-moz-range-thumb{width:16px;height:16px;border-radius:50%;background:#ffffff;border:none}
.inline-value{color:#fff;font-size:13px}

/* Виджеты */
.widget-row{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:10px}
...
.linklike.same{font-size:13px} /* одинаковый размер с текстом чеков */
...
.sla-title{margin:0 0 6px}
.price{margin:0;color:#fff}
.price-note{color:#c0c0c0;font-size:16px;margin-bottom:8px}
.sla-groups{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:6px}
.sla-group{background:#0d0f12;border:1px solid var(--line);border-radius:12px;padding:12px}
...
.white-btn{background:#ffffff;color:#000;border-color:#ffffff;margin-top:8px}
.cta-row{display:grid;grid-template-columns:1fr;gap:8px;margin-top:10px}

/* Модальные окна (Apple surfaces) */
.pricing-modal-overlay{
  position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(10px);display:flex;align-items:center;justify-content:center;z-index:10000;padding:20px
}
.pricing-modal-window{
  background:#f5f5f7;border-radius:28px;width:880px;max-width:100%;max-height:90vh;box-shadow:0 20px 60px rgba(0,0,0,0.5);
...
.pricing-modal-body{padding:0 80px 60px;overflow:auto}
...
.owner-grid.spaced{gap:24px} /* больше расстояний между 4 блоками */
.owner-col{display:grid;gap:10px}
.cat-h2{font-size:19px;font-weight:600;color:#1d1d1f;margin:0}
.badge{background:#e3e4e7;border:1px solid #d2d3d6;border-radius:999px;padding:6px 10px;font-weight:600;font-size:12px;width:max-content}
...
/* Ползунки в модалках такие же белые */
...
@media (max-width:1024px){
  ...
  .pricing-modal-header, .pricing-modal-title, .pricing-modal-body{margin-left:24px;margin-right:24px;padding:0 24px 40px}

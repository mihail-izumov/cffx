<template>
  <div class="consultation-section">
    <div class="consultation-container">
      <p class="consultation-description">Увидеть свой бизнес глазами клиентов</p>
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            <select id="tariff" name="tariff" class="form-select" required>
              <option value="">Тариф для старта</option>
              <option value="basic">Базовый</option>
              <option value="advanced">Расширенный</option>
            </select>
            <div id="tariffHint" class="tariff-hint"></div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    initForm() {
      ...
      // Подсказки по тарифам
      const tariffHints = {
        'basic': 'Забрать гостей у конкурентов',
        'advanced': 'Создавать новые правила игры на рынке'
      };
      
      // Обработчик изменения тарифа
      tariffSelect.addEventListener('change', (e) => {
        const selectedValue = e.target.value;
        const hint = tariffHints[selectedValue] || '';
        tariffHint.textContent = hint;
        checkFormValidity();
      });
      ...
      const checkFormValidity = () => {
        ...
        const tariffValid = tariffSelect.value !== '';
        const consentValid = checkbox.checked;
        
        submitBtn.disabled = !(nameValid && phoneValid && emailValid && tariffValid && consentValid);
      };
      ...
      form.addEventListener('submit', (e) => {
        ...
        .then(response => {
          ...
          form.reset();
          tariffHint.textContent = ''; // Очищаем подсказку
        ...
        .catch(error => {
          ...
          const mailtoBody = `Имя: ${formData.name}%0AТелефон: ${formData.phone}%0AEmail: ${formData.email}%0ATариф: ${formData.tariff}`;
          window.location.href = `mailto:theorchestramanco@gmail.com?subject=Подключить Радар&body=${mailtoBody}`;
        })
        .finally(() => {
          // Восстанавливаем кнопку
          submitBtn.textContent = 'Подключить Радар';

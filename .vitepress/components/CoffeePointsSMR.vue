    ...
    <div v-if="!showBranchList">
      ...
      <div class="signal2-main-card">
        <div class="signal2-establishment-header">
          ...
          <div class="signal2-status-badge" v-if="establishment.value.status">{{ establishment.value.status }}</div>
        </div>
...
/* ---- далее повторяются все стили с оригинальным префиксом signal2- вместо review2- ---- */
/* Просто по шаблону выше заменён префикс review2- -> signal2- для полной изоляции и будущих стилизаций */
/* ... Все стили ниже с заменённым префиксом ... */
/* Основные контейнеры */
.signal2-widget-header, .signal2-branches-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;}
.signal2-header-title, .signal2-branches-title { margin:0; color:white; font-size:26px; font-weight:700; line-height:1.2; text-align:left; flex-grow:1;}
.signal2-header-subtitle { margin-top:8px; font-size:15px; color:var(--vp-c-text-2);}
.signal2-branches-header { padding-bottom:20px; border-bottom:2px solid var(--vp-c-border);}
.signal2-branches-title { text-align:center;}
...
.signal2-close-btn:hover { background:linear-gradient(135deg,#991b1b,#ef4444); border-color:#ef4444; color:white; transform:rotate(90deg);}
...
.signal2-branches-card { --signal2-border-gradient: linear-gradient(135deg,#3730a3,#8b5cf6,#c4b5fd); --signal2-glow-color: rgba(139,92,246,0.25); --signal2-glow-hover-color: rgba(139,92,246,0.6);}
.signal2-index-card { --signal2-border-gradient: linear-gradient(135deg,#4d7c0f,#a3e635,#C5F946); --signal2-glow-color: rgba(197,249,70,0.25); --signal2-glow-hover-color: rgba(197,249,70,0.6);}
.signal2-reviews-card { --signal2-border-gradient: linear-gradient(135deg,#b45309,#f59e0b,#fcd34d); --signal2-glow-color: rgba(245,158,11,0.25); --signal2-glow-hover-color: rgba(245,158,11,0.6);}
...
.signal2-system-status-bar { display:flex; align-items:center; justify-content:center; gap:12px; margin:20px 0 16px 0; padding:10px 16px; background:rgba(255,255,255,0.03); border-radius:12px; border:1px solid rgba(255,255,255,0.06);}
.signal2-status-label { font-size:12px; font-weight:600; color:rgba(255,255,255,0.7); margin-right:8px; flex-shrink:0;}
...
.signal2-metric-time { font-size:12px; font-weight:700; color:rgba(255,255,255,0.9); font-family:'SF Mono','Monaco','Inconsolata',monospace; min-width:32px; text-align:right; transition:all 0.3s ease;}
.signal2-metric-text { font-size:11px; font-weight:500; color:rgba(255,255,255,0.6);}
.signal2-status-separator { color:rgba(255,255,255,0.3); font-size:12px; margin:0 4px;}
...
.signal2-button-icon { transition:transform 0.3s ease;}
.signal2-review-button:hover .signal2-button-icon { transform:translateX(4px);}
...
.signal2-modal-link { color:#A3E635; text-decoration:none; font-weight:600; transition:color 0.3s ease;}
.signal2-modal-link:hover { color:#C5F946; text-decoration:underline;}
...
@media (max-width: 768px) {
  .signal2-widget-content { padding:24px;}
  .signal2-main-card { padding:16px;}
  .signal2-stats-grid { grid-template-columns:1fr; gap:12px;}
  .signal2-stat-card { display:flex; flex-direction:row; align-items:center; border-radius:16px; transition:none;}
  ...
  .signal2-system-status-bar { flex-direction:column; gap:8px; padding:12px;}
  .signal2-status-metrics { gap:12px;}
}
@media (max-width: 480px) {
  .signal2-widget-content { padding:20px;}
  .signal2-header-title { font-size:22px; text-align:left;}
  .signal2-header-subtitle { font-size:14px; }
  .signal2-branches-title { font-size:22px; text-align:center;}
  ...
  .signal2-metric-time { font-size:11px; min-width:28px;}
  .signal2-metric-text { font-size:10px;}

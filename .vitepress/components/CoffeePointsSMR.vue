...
<template>
  <div class="review2-widget-content" ref="widgetContentRef">
    <!-- Переключатели кофеен -->
    <div class="cafe-switchers" style="display: flex; gap: 8px; margin-bottom: 18px;">
      <button 
        v-for="name in cafeNames"
        :key="name"
        class="review2-switcher"
    ...
    <div v-if="!showBranchList">
      ...
      <div class="review2-main-card">
        <div class="review2-establishment-header">
          <h3 class="review2-cafe-name">{{ establishment.value.name }}</h3>
          <div class="review2-status-badge" v-if="establishment.value.status">
            {{ establishment.value.status }}
          </div>
        </div>
        <div class="review2-stats-grid">
          <div class="review2-stat-card review2-branches-card">
            <div class="review2-stat-content">
              <div class="review2-stat-left-group">
                ...
                <div class="review2-stat-value">{{ establishment.value.branches.length }}</div>
              </div>
          ...
          <div class="review2-stat-card review2-index-card">
            <div class="review2-stat-content">
              <div class="review2-stat-left-group">
                <div class="review2-stat-icon">⚡</div>
                <div class="review2-stat-value">{{ establishment.value.index }}</div>
              </div>
          ...
          <div class="review2-stat-card review2-reviews-card">
            <div class="review2-stat-content">
              <div class="review2-stat-left-group">
                <div class="review2-stat-icon">📡</div>
                <div class="review2-stat-value">{{ establishment.value.totalReviews }}</div>
              </div>
    ...
    <div v-else>
      <div class="review2-branches-header">
        ...
        <h2 class="review2-branches-title">{{ establishment.value.name }}</h2>
      ...
      <div class="review2-branches-content">
        ...
        <div class="review2-branches-list">
          <button v-for="(branch, index) in establishment.value.branches" :key="index" @click="goToReviews(branch)" class="review2-branch-item">
...
.review2-switcher.active {
  background: #f59e0b;
  color: #222;
}
/* --- полный блок оригинального стиля из вашего вопроса --- */
/* Основные контейнеры */

    ...
    <form v-else @submit.prevent="submitForm">
      ...
      <div class="signal-form-section">
        <div class="signal-question-block signal-compact">
          <label class="signal-question-label">Ваш фитнес-клуб "СуперСпорт"</label>
      ...
      <div v-if="showInfoModal" class="modal-overlay" @click.self="showInfoModal = false">
        <div class="modal">
          ...
          <div class="modal-body">
            Каждый отзыв делает любимый фитнес-клуб еще лучше, а Сигнал помогает решить Вашу проблему за 24 часа. Почувствуйте силу настоящих перемен.<br><br>
...
const fitnessContent = {
  ...
  suggestions: {
    emotions: {
      ...
      'все смотрят': ['пристальные взгляды', 'комментарии', 'смех над новым клиентом', 'игнор новичков', 'незащищённость'],
...
// === ВОПРОСЫ ===
const phrasesForQuestion1 = fitnessContent.questions[1];
const phrasesForQuestion2 = fitnessContent.questions[2];
const phrasesForQuestion3 = fitnessContent.questions[3];

let currentQuestion1 = ref(phrasesForQuestion1[0]);
let currentQuestion2 = ref(phrasesForQuestion2[0]);
let currentQuestion3 = ref(phrasesForQuestion3[0]);
...
const suggestions = computed(() => ({
  emotions: fitnessContent.suggestions.emotions,
  facts: fitnessContent.suggestions.facts,
  solutions: fitnessContent.suggestions.solutions
}));
...
function isInitialSuggestions(type) {
  return JSON.stringify(currentSuggestions[type]) === JSON.stringify(suggestions.value[type].initial);
}

function resetSuggestions(type) {
  currentSuggestions[type] = [...suggestions.value[type].initial];
}
...
function updateSuggestions(type, word) {
  const next = suggestions.value[type][word];
  currentSuggestions[type] = next && next.length ? [...next] : [...suggestions.value[type].initial];
}
...
async function submitForm() {
  ...
  const now = new Date();
  submittedTime.value = now.toISOString().slice(0, 19).replace('T', ' ');
  ...
  try {
    ...
    alert(error.message?.includes('много запросов')
...
onMounted(() => {
  ...
  const now = new Date();
  currentDate.value = now.toISOString().slice(0, 19).replace('T', ' ');

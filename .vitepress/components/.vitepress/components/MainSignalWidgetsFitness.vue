...
// ============ ПРОФИЛИ ФИТНЕС КЛУБОВ ============
const fitnessProfiles = {
  'world class': {
    responseTime: { base: 2.0, min: 1.5, max: 2.5 },
    resolutionTime: { base: 16.0, min: 14, max: 18 }
  },
  'fiz культура': {
    responseTime: { base: 1.8, min: 1.2, max: 2.4 },
    resolutionTime: { base: 15.0, min: 13, max: 17 }
  },
  'x-fit': {
    responseTime: { base: 2.2, min: 1.8, max: 2.6 },
    resolutionTime: { base: 17.0, min: 15, max: 19 }
  },
  'smstretching': {
    responseTime: { base: 1.0, min: 0.8, max: 1.2 },
    resolutionTime: { base: 12.0, min: 10, max: 14 }
  },
  'sportlife': {
    responseTime: { base: 1.2, min: 1.0, max: 1.5 },
    resolutionTime: { base: 13.0, min: 11, max: 15 }
  },
  'fitness house': {
    responseTime: { base: 0.9, min: 0.7, max: 1.1 },
    resolutionTime: { base: 11.0, min: 9, max: 13 }
  },
  'ddx': {
    responseTime: { base: 0.8, min: 0.6, max: 1.0 },
    resolutionTime: { base: 10.0, min: 8, max: 12 }
  }
}
...
const getFitnessConfig = (fitnessName) => {
  const normalized = fitnessName?.toLowerCase() || ''
  return fitnessProfiles[normalized] || {
    responseTime: { base: 1.5, min: 1, max: 2 },
    resolutionTime: { base: 14, min: 12, max: 16 }
  }
}

const selectedFitness = ref(fitnessNames[0] || 'World Class')

const establishment = computed(() => fitnessClubs[selectedFitness.value] || {
  name: '',
  image: '',
  ListeningStatus: '',
  ListeningBadgeText: '',
...
const fitnessConfig = computed(() => getFitnessConfig(selectedFitness.value))

const systemMetrics = ref({
  responseTime: fitnessConfig.value.responseTime.base,
  resolutionTime: fitnessConfig.value.resolutionTime.base,
  lastUpdate: Date.now()
})
...
const handleSwipe = () => {
  ...
  if (Math.abs(diffX) > 50) {
    const currentIndex = fitnessNames.indexOf(selectedFitness.value)
    let newIndex

    if (diffX > 0) {
      newIndex = currentIndex - 1
      if (newIndex < 0) newIndex = fitnessNames.length - 1
    } else {
      newIndex = currentIndex + 1
      if (newIndex >= fitnessNames.length) newIndex = 0
    }

    selectedFitness.value = fitnessNames[newIndex]
  }
}
// --- КОНЕЦ ЛОГИКИ СВАЙПА ---
...
const fetchSystemStatus = async () => {
  try {
    ...
    const currentConfig = fitnessConfig.value
    const now = Date.now()
...
const rotatingQuestions = [
  ...
  '\"Что дало ощущение комфорта/дискомфорта?\"',
  '\"Одно слово, которое осталось после тренировки?\"'
]
...
const preloadImages = () => {
  const imagesToPreload = Object.values(fitnessClubs)
    .map(fitness => fitness.image)
    .filter(Boolean)
...
watch(selectedFitness, (newName) => {
  const newConfig = getFitnessConfig(newName)
...
<template>
  <div class="signal2-widget-content" ref="widgetContentRef">
    <div class="signal2-cafe-switchers-container">
      ...
      >
        <button
          v-for="name in fitnessNames"
          :key="name"
          class="signal2-switcher"
          :class="{ active: selectedFitness === name }"
          @click="selectedFitness = name"
        >
          {{ name }}
        </button>
      </div>
    ...
    >
      ...
      >
        ...
        <div class="signal2-modal-body">
          ...
          поступает в работу управляющему клуба, и вы получаете не просто
...
<style scoped>
/* Все стили остаются те же из оригинального кода */
.fade-enter-active,
.fade-leave-active {

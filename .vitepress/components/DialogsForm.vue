    ...
    <div class="consultation-container">
      ...
      <form id="consultationForm" class="consultation-form">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            <select id="tariff" name="tariff_plan" class="form-select" required>
              <option value="">Тариф для старта</option>
              <option value="basic">Базовый</option>
              <option value="advanced">Продвинутый</option>
              <option value="full">Полный контроль</option>
            </select>
            <div id="tariffHint" class="investment-hint"></div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    initForm() {
      ...
      // Подсказки для тарифов
      const tariffHints = {
        'basic': 'Реакция на отзывы',
        'advanced': 'Предвосхищение проблем',
        'full': 'Стратегическое управление репутацией'
      };
      ...
      form.addEventListener('submit', (e) => {
        ...
        const formData = {
          ...
          tariff_plan: form.tariff_plan.value,
          consent: checkbox.checked ? 'Да' : 'Нет',
          subject: 'Подключить Диалоги'
        };
        ...
        .catch(error => {
          ...
          const mailtoBody = `Имя: ${formData.name}%0AТелефон: ${formData.phone}%0AEmail: ${formData.email}%0AТариф: ${formData.tariff_plan}`;
          window.location.href = `mailto:theorchestramanco@gmail.com?subject=Подключить Диалоги&body=${mailtoBody}`;
        })
        .finally(() => {

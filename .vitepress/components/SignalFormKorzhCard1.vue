...
// ПОЛНАЯ 3-УРОВНЕВАЯ система подсказок с гендерными вариантами для ПОЗДРАВЛЕНИЙ
const baseSuggestions = {
  female: {
    emotions: {
      ...
      // LEVEL 2 (FEMALE)
      'поздравляю': ['вас', 'всех', 'этот мир', 'команду', 'себя', 'любимую кофейню'],
      'желаю': ['лёгкости', 'вдохновения', 'чуда', 'денег', 'любви'],
      'шлю': ['сигнал в космос', 'сердечко', 'вайб успеха', 'привет', 'лучи добра'],
      'дарю': ['улыбку', 'лайк', 'этот момент', 'свое восхищение', 'прощение'],
      'наслаждаюсь': ['тишиной', 'свободой', 'вкусом', 'этой зимой', 'моментом'],
      'ловлю': ['дзен', 'инсайт', 'удачу', 'снежинки', 'вдохновение'],
      'официально заявляю': ['мой девиз', 'я на диете', 'дедлайны горят', 'я счастлива', 'мне вкусно'],
      'признаюсь': ['я влюбилась', 'согрешили с десертом', 'без кофе кусаюсь', 'режим зомби вкл', 'патчи не спасли']
    }
  },
  male: {
    emotions: {
      ...
      // LEVEL 2 (MALE)
      'поздравляю': ['вас', 'всех', 'этот мир', 'команду', 'себя', 'любимую кофейню'],
      'желаю': ['мощи', 'прорыва', 'успеха', 'драйва', 'спокойствия'],
      'шлю': ['сигнал в космос', 'респект', 'вайб успеха', 'салют', 'лучи добра'],
      'дарю': ['лайк', 'пять звезд', 'благодарность', 'рекомендацию', 'отзыв'],
      'кайфую от': ['тишины', 'атмосферы', 'вкуса', 'этой паузы', 'музыки'],
      'ловлю': ['дзен', 'инсайт', 'удачу', 'поток', 'идеи'],
      'официально заявляю': ['мой девиз', 'у меня растущий организм', 'дедлайны горят', 'я заряжен', 'кофе топ'],
      'признаюсь': ['это шедевр', 'съел два десерта', 'спасся кофеином', 'не хочу работать', 'залип тут']
    }
  },
  ...
  common: {
    emotions: {
      // === ГРУППА "ПОЗДРАВЛЯЮ" ===
      'любимую кофейню': ['с Новым годом', 'с днем рождения', 'с классным сервисом', 'с расширением'],
      'команду': ['вы супер', 'так держать', 'с праздником', 'спасибо за труд'],
      'себя': ['с перерывом', 'с бодрым утром', 'с правильным выбором', 'с достижениями'],
      'вас': ['с наступающим', 'с новым днем', 'от души', 'просто так', 'сердечно'],
      'всех': ['Жить любить кофе пить', 'с праздником', 'вокруг', 'с новой жизнью', 'благ земных'],
      ...
      'вдохновения': ['творить', 'на новые свершения', 'жить ярко', 'каждый день', 'и креатива'],
      'чуда': ['новогоднего', 'в каждом дне', 'своими руками', 'внезапного', 'волшебного'],
      'денег': ['чемодан', 'на все мечты', 'легких', 'мешок', 'и власти'],
      'любви': ['к себе', 'безусловной', 'взаимной', 'к кофе', 'и тепла'],
      ...
      'спокойствия': ['и уюта', 'в душе', 'как здесь', 'вечного'],

      // === ГРУППА "ШЛЮ / ДАРЮ" ===
      'сигнал в космос': ['все будет супер', 'я здесь', 'прием', 'желание отправлено'],
      'сердечко': ['команде', 'бариста', 'этому городу', 'всем', 'друзьям'],
      'вайб успеха': ['заразительный', 'для всех', 'мощный', '100%', 'бесплатный'],
      'привет': ['друзьям', 'маме', 'подписчикам', 'из кофейни', 'всем своим'],
      ...
      'улыбку': ['вам в ответ', 'на удачу', 'всему миру', 'на память', 'широкую'],
      'лайк': ['не глядя', 'за атмосферу', 'от души', 'вашему кофе'],
      'этот момент': ['в копилку', 'на память', 'тебе', 'миру'],
      'свое восхищение': ['мастерством', 'вкусом', 'сервисом', 'интерьером'],
      'прощение': ['за съеденный круассан', 'за опоздание', 'всем бывшим', 'себе'],
      'пять звезд': ['в рейтинг', 'заслуженно', 'за сервис', 'без вопросов'],
      'благодарность': ['за уют', 'за вкус', 'за прием', 'безмерную'],
      'рекомендацию': ['друзьям', 'коллегам', 'в сторис', 'от сердца'],
      'отзыв': ['честный', 'позитивный', 'на память', 'в книгу жалоб'],

      // === ГРУППА "НАСЛАЖДАЮСЬ / КАЙФУЮ" ===
      'тишиной': ['внутри меня', 'посреди шума', 'и покоем', 'перед стартом', 'в мыслях'],
      'свободой': ['от суеты', 'от телефона', 'мыслить', 'быть собой', 'выбора'],
      'вкусом': ['арабики', 'десерта', 'настоящим', 'любимым', 'жизни'],
      'этой зимой': ['и снегом', 'из окна', 'уютной', 'сказочной', 'теплой'],
      'моментом': ['здесь и сейчас', 'пока кофе горячий', 'остановки времени', 'истины'],
      'атмосферой': ['уюта', 'праздника', 'творчества', 'доверия'],
      'этой паузы': ['в делах', 'для себя', 'необходимой', 'сладкой'],
      'музыки': ['в наушниках', 'фоновой', 'любимой', 'в душе'],

      // === ГРУППА "ЛОВЛЮ" ===
      'дзен': ['в чашке кофе', 'посреди хаоса', 'и гармонию', 'буддиста', 'сейчас'],
      'инсайт': ['гениальный', 'внезапно', 'под кофе', 'для бизнеса', 'о жизни'],
      'удачу': ['за хвост', 'в делах', 'сегодня', 'крупную'],
      'снежинки': ['ртом', 'взглядом', 'на окне', 'настроения'],
      'вдохновение': ['(это винительный падеж)', 'творить', 'из воздуха', 'большое'],
      'поток': ['энергии', 'творчества', 'мыслей', 'созидания', 'денежный'],
      'идеи': ['на миллион', 'для стартапа', 'свежие', 'крутые'],

      // === ГРУППА "ОФИЦИАЛЬНО ЗАЯВЛЯЮ" ===
      ...
      'дедлайны горят': ['а я пью кофе', 'но пусть подождут', 'гори они огнем', 'спасаюсь здесь'],

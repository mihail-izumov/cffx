<template>
  <div class="form-wrapper">
    <!-- ИСПРАВЛЕННЫЙ экран успешной отправки - только один чекмарк -->
...
/* ПОЛНОСТЬЮ ПЕРЕПИСАННЫЕ стили для экрана успешной отправки */
...
/* КРИТИЧЕСКИ ВАЖНО: убираем ВСЕ псевдоэлементы */
.success-icon, .success-icon *, .success-icon *::before, .success-icon *::after { content: none !important; }
.success-icon::before, .success-icon::after { content: none !important; display: none !important; visibility: hidden !important; }

.success-icon { width: 64px; height: 64px; border-radius: 50%; background-color: #4CAF50; display: flex; align-items: center; justify-content: center; margin-bottom: 1.5rem; animation: popIn 0.5s 0.2s cubic-bezier(0.25, 0.46, 0.45, 0.94) backwards; position: relative; overflow: hidden; }
.success-icon svg { width: 32px; height: 32px; color: white; position: relative; z-index: 2; }

<template>
  <!-- SVG DEFINITIONS — ГЛОБАЛЬНЫЕ ДЛЯ ВСЕХ ИКОНОК -->
  <svg width="0" height="0" style="position: absolute; pointer-events: none;">
    <defs>
      <!-- ГРАДИЕНТ 1: Диагональ ↘ (светлый → тёмный) -->
      ...
      <!-- ГРАДИЕНТ 2: Диагональ ↙ (тёмный → светлый) -->
      <linearGradient id="icon-gradient-2" x1="100%" y1="0%" x2="0%" y2="100%">
  ...
  <div class="sss-slider-wrapper">
    <div class="sss-brands-container" ref="sliderContainer">
      <a
        ...
        role="link"
      >
        <!-- SVG С ЭФФЕКТОМ ОБЪЁМА -->
        <div class="sss-card-background-icon" :class="{ 'hover': activeSlide === idx }">
          <svg 
            xmlns="http://www.w3.org/2000/svg" 
            viewBox="0 0 24 24" 
            fill="none" 
            :stroke="idx % 2 === 0 ? 'url(#icon-gradient-1)' : 'url(#icon-gradient-2)'"
            stroke-width="1.8" 
            stroke-linecap="round" 
            stroke-linejoin="round"
            :filter="activeSlide === idx ? 'url(#depth-effect-hover)' : 'url(#depth-effect)'"
          >
...
const slides = [
  { headline: 'Обратная связь за пару кликов', benefit: 'Гость легко и быстро сообщает о проблеме, а вы получаете все обращения точно и вовремя.', control: 'Виджет работает на сайте, в приложении или в любом формате для сферы услуг.', details: 'Любой клиент фитнеса или гость ресторана оставляет обратную связь за пару кликов — Сигнал или отзыв в Яндекс/2ГИС. Если гость недоволен, Сигнал сразу идёт команде, а не в открытые рейтинги. Положительные отклики легко отправляются на агрегаторы.', paths: ['M14 4.1 12 6','m5.1 8-2.9-.8','m6 12-1.9 2','M7.2 2.2 8 5.1','M9.037 9.69a.498.498 0 0 1 .653-.653l11 4.5a.5.5 0 0 1-.074.949l-4.349 1.041a1 1 0 0 0-.74.739l-1.04 4.35a.5.5 0 0 1-.95.074z'] },
  { headline: 'Умная форма', benefit: 'Вы получаете готовую форму с цепочками сценариев, согласование — 1 день, без погружения в технологию.', control: 'Настраиваем форму под специфику бизнеса — структуру Сигналов, формулировки, точки напряжения.', details: 'Форма построена на 150+ уникальной цепочке — для каждого случая есть свой сценарий, три уровня детализации и автоматическая маршрутизация. Не отзыв, а Сигнал — 30 секунд, система сама подсказывает сценарий.', rect: { w: '18', h: '14', x: '3', y: '5', rx: '2', ry: '2' }, paths: ['M7 15h4','M15 15h2','M7 11h2','M13 11h4'] },
  { headline: 'Тикеты в Телеграм', benefit: 'Все задачи решаются вовремя, рекомендации работают, ошибки исключены.', control: 'Видите статус, сроки, причину проблемы и итог — система концентрируется на пользу для гостя.', details: 'Каждый Сигнал превращается в тикет с рекомендацией, маршрутом и сроком решения. Тикеты хранят всю историю коммуникации, решение и удовлетворённость гостя. Вместо CRM — удобные тикеты в Телеграм: 1 Сигнал = 1 тикет, всё на своем месте, это комфортно для команды.', paths: ['M22 17a2 2 0 0 1-2 2H6.828a2 2 0 0 0-1.414.586l-2.202 2.202A.71.71 0 0 1 2 21.286V5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2z','M7 11h10','M7 15h6','M7 7h8'] },
  { headline: 'Поддержка оператора', benefit: 'Ваша команда не теряет время на рутину — оператор берёт на себя обработку, координацию и контроль, вы фокусируетесь на развитии сервиса.', control: 'Оператор работает по правилам и срокам вашего бизнеса.', details: 'Оператор ведёт каждый Сигнал с самого начала — строит сценарий, контролирует маршрут, следит за сроками, поддерживает ваши стандарты. Режим работы: в будни 9-18 или 24/7, если требуется максимальное покрытие.', paths: ['M3 11h3a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-5Zm0 0a9 9 0 1 1 18 0m0 0v5a2 2 0 0 1-2 2h-1a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h3Z','M21 16v2a4 4 0 0 1-4 4h-5'] },
...
.sss-card-background-icon.hover,
.sss-brand-card:hover .sss-card-background-icon {
  opacity: 0.22 !important;
  transform: translateY(-2px) scale(1.08) !important;
}
.sss-card-background-icon.hover svg,
.sss-brand-card:hover .sss-card-background-icon svg {
